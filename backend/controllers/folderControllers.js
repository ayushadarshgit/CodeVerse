...
module.exports.getFolderDetails = async (req, res) => {
    ...
    if (!searchedFolder.isdefaultfolder && !searchedFolder.owner._id.equals(req.user._id)) {
...
module.exports.createFolder = async (req, res) => {
    ...
    for (let folder of rootFolder.folders) {
        if (folder.foldername === req.body.folder.foldername) {
            throw new ExpressError(`Folder with name ${folder.foldername} already exists in the root folder`, 400);
        }
    }
...
module.exports.getParentFolder = async (req, res) => {
    const { folderId } = req.body;
    if (!folderId) {
        throw new ExpressError("FolderId Not provided", 400);
    }
    const folder = await Folder.findById(folderId)
        .populate("owner");
    if (!folder.owner._id.equals(req.user._id)) {
        throw new ExpressError("You are not the Owner of this folder", 400);
    }
    const parent = await Folder.findOne({
        folders: { $elemMatch: { $eq: folderId } }
    })
        .populate("folders")
        .populate("files");
    return res.json({ success: true, parent: parent });
}
...
module.exports.deleteFolder = async (req, res) => {
    ...
    if (!f.owner._id.equals(req.user._id)) {
